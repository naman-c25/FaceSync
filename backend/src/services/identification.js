...
export async function identifyFromSession(session, { completeOnMatch = true } = {}) {
  ...
  // The second of the two places a verification log is written, so the second
...
export function livenessFields(signals, { passed = true, failureReason = null } = {}) {
  ...
  return {
    ...
    spoofCropScale: signals.spoof_crop_scale ?? null,
    continuityScore: signals.continuity_score ?? null,
    bestFrameSharpness: signals.best_frame_sharpness ?? null,
  };
}
